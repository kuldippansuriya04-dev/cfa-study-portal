...
export function useSubmitPracticeAnswer() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      const isCorrect =
        !answerKeyMissing && question?.correctAnswer === payload.selectedAnswer;
      ...
      if (answerKeyMissing) return attempt;
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      for (const a of payload.answers) {
        ...
        if (a.selectedAnswer && q?.correctAnswer === a.selectedAnswer)
          correctCount++;
      }
      const totalQCount = exam?.questionCount ?? payload.answers.length;
      const score =
        totalQCount > 0 ? Math.round((correctCount / totalQCount) * 100) : 0;
