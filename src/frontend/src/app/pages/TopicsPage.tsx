...
import { Clock, BookOpen, ArrowRight, PencilLine } from "lucide-react";
...
export function TopicsPage({ onNavigate }: TopicsPageProps) {
  const practiceTopicId = (topicId: string) =>
    topicId === "alt" ? "alts" : topicId;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          CFA Level I covers 10 topic areas. Click a topic to see readings and
          start studying.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {TOPICS.map((topic) => {
          const questionCount = PRACTICE_PACK_QUESTIONS.filter(
            (question) => question.topicId === practiceTopicId(topic.id),
          ).length;
          return (
            <Card
              key={topic.id}
            ...
            >
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-center gap-3 mb-3 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1">
                    <PencilLine size={11} /> {questionCount} Qs
                  </span>
                  <span className="flex items-center gap-1">
