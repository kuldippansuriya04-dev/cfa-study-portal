...
import { ArrowLeft, Clock, BookOpen, BarChart2, ChevronRight, CheckCircle2 } from 'lucide-react';
...
export function TopicDetailPage({ topicId, onNavigate }: TopicDetailPageProps) {
    ...
    return (
      <div className="flex items-center justify-center h-full p-8">
        <div className="text-center">
          ...
          <Button variant="ghost" size="sm" className="mt-4" onClick={() => onNavigate('topics')}>
            ← Back to Topics
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      {/* Back button */}
      <Button variant="ghost" size="sm" className="text-muted-foreground -ml-2" onClick={() => onNavigate('topics')}>
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <p className="text-white/70 text-xs font-semibold uppercase tracking-wide">{topic.code}</p>
                <h1 className="text-xl font-bold leading-tight">{topic.name}</h1>
              </div>
            </div>
            <p className="text-white/80 text-sm leading-relaxed max-w-xl">{topic.description}</p>
          </div>
          <div className="flex-shrink-0">
            ...
            >
              <BookOpen size={14} className="mr-1" /> Practice Quiz
            </Button>
          </div>
        </div>
        <div className="flex flex-wrap gap-3 mt-4">
          {[
            ...
            <div key={label} className="flex items-center gap-1.5 bg-white/20 rounded-full px-3 py-1 text-xs font-medium">
              <span>{icon}</span>
              <span>{label}</span>
            </div>
          ))}
        ...
        <CardContent className="p-6 pt-0 space-y-2">
          ...
          {modules.map((mod) => {
            ...
            return (
              <div
                key={mod.number}
                onClick={() => onNavigate(`reading-${topicId}-${mod.number}` as PageId)}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${isDone ? 'text-green-700' : 'text-foreground'}`}>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-[10px] text-muted-foreground">{mod.pages} pages</span>
                    <span className="text-[10px] text-muted-foreground">·</span>
                    <span className="text-[10px] text-muted-foreground">📄 PDF Content</span>
                    {isDone && (
                ...
                >
                  <CheckCircle2
                    size={18}
                    className={isDone ? 'text-green-500' : 'text-muted-foreground/40 hover:text-green-400'}
                  />
                </button>
